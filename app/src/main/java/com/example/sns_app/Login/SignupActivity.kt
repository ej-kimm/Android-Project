...
class SignupActivity : AppCompatActivity() {
    ...
    lateinit var userInformationData: UserInformationData // 유저 정보들을 담을 data class
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnSignupBtn.setOnClickListener {
            ...
            userInformationData = UserInformationData(signup_nickname, signup_password, signup_birth, signup_email, userID) // UserInformation 데이터 클래스에 담음
            if(signup_nickname.isNotEmpty() && signup_password.isNotEmpty() && signup_birth.isNotEmpty() && signup_email.isNotEmpty()) { // 공백이 아닐 경우에만 가입
                if (canSignup(userInformationData)) { // 제약조건 확인
                    doSignUp(userInformationData)
                }
            }
        }
    }
    ...
    private fun canSignup(users: UserInformationData): Boolean {
